...
export class TestEntityComponent implements OnInit {
  ...
  testEntities = signal<ITestEntity[]>([]);
  isLoading = false;

  sortState = sortStateSignal({});

  public readonly router = inject(Router);
  protected readonly testEntityService = inject(TestEntityService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: ITestEntity): number => this.testEntityService.getTestEntityIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.testEntities().length === 0) {
            ...
            this.testEntities.set(this.refineData(this.testEntities()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.testEntities.set(this.refineData(dataFromBody));
  }
